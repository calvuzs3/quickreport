...
/**
 * Updates an existing criticality level after validating required fields and code
 * uniqueness (excluding the record being edited).
 */
class UpdateCriticalityLevelUseCase @Inject constructor(
    ...
    suspend operator fun invoke(level: CriticalityMaster): QrResult<Unit, QrError> {
        ...
        if (existing != null && existing.id != level.id) {
            return QrResult.Error(QrError.ValidationError.InvalidOperation())
        }

        return repository.updateCriticalityLevel(level).fold(
            onSuccess = { QrResult.Success(Unit) },
            onFailure = {
                Timber.e(it, "Failed to update criticality level ${level.id}")
