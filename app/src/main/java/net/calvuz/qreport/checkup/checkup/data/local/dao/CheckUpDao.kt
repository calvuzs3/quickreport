...
interface CheckUpDao {
    ...
    /**
     * Bumps updated_at without touching any other field — used to mark the
     * checkup pending-sync when a child record that isn't itself synced
     * per-row changes (e.g. a photo insert/delete, see
     * [net.calvuz.qreport.checkup.checkup.domain.usecase.TouchCheckUpForCheckItemUseCase]).
     */
    @Query("UPDATE checkups SET updated_at = :updatedAt WHERE id = :id")
    suspend fun touchUpdatedAt(id: String, updatedAt: Instant)
