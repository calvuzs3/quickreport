...
/**
 * SHA-256 hash utility for photo file content. Mirror of
 * [net.calvuz.qreport.client.document.sync.DocumentHash] — kept as a
 * separate copy so the photo sync package doesn't depend on the document
 * one for an unrelated concern.
 *
...
object PhotoHash {
    ...
    suspend fun verify(filePath: String, expectedHash: String): Boolean {
        val actual = runCatching { compute(filePath) }.getOrElse { e ->
            Timber.e(e, "PhotoHash: failed to compute hash for $filePath")
            return false
        }
        val match = actual == expectedHash
        if (!match) {
            Timber.w(
                "PhotoHash: mismatch for $filePath — " +
